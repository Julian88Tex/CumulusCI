...
class SetFieldHelpText(MetadataSingleEntityTransformTask):  
    ...
    def _init_options(self, kwargs):  
        ...
        if (  
            ...
            raise TaskOptionsError(  
                "Please populate the fields field with a list of dictionaries containing at "
                "minimum one entry with an 'api_name' and 'help_text' keys"
            )  
        ...
        if not all(["help_text" in entry for entry in self.options["fields"]]):  
            raise TaskOptionsError(  
                "The 'help_text' key is required on all entry values to declare what help "
    ...
    def _transform_entity(self, metadata: MetadataElement, api_name: str):  
       ...
       # Quoting the updated code for citation
quote("from collections import defaultdict", "_modify_help_text(metadata, api_name, field, help_text)")
